...
class LaTeXBase:
    ...
    def parse_latex_expression(latex_str: str, variable_values: Dict[str, float] = None) -> sp.Expr:
        ...
        try:
            ...
            raise ValueError(f"Can't parse LaTeX expression: {e}")
...
def print_help():
    ...
    combined_table = Table(
        title="LaTeX Syntax & Commands",
        header_style="bold",
        border_style="default"
    )
    combined_table.add_column("Category", style="cyan", width=25)
    combined_table.add_column("Details", style="green", width=40)
    combined_table.add_column("Example/Description", style="yellow")

    combined_table.add_row("Supported LaTeX Syntax", "Arithmetic", "+, -, \\times, \\cdot, \\div")
    ...
    combined_table.add_row("", "Exponents", "^{exponent}")
    combined_table.add_row("", "Functions", "\\sin, \\cos, \\tan, \\log, \\ln, \\exp, etc.")
    ...
    combined_table.add_row("Equation Solving", "Single Equation", "x^2 - 4 = 0")
    combined_table.add_row("", "System of Equations", "x + y = 5, x - y = 1 (use comma to separate equations)", end_section=True)

    combined_table.add_row("Calculus", "Derivative", "Input expression, optional variable (default x)")
    combined_table.add_row("", "Integral", "Input expression, optional variable (default x)")
    combined_table.add_row("", "Limit", "Input expression, optional variable and point (default x→0)", end_section=True)

    combined_table.add_row("Commands", "help", "Display this help")
    combined_table.add_row("", "solve", "Enter equation solving mode")
    combined_table.add_row("", "calc", "Return to calculation mode")
    combined_table.add_row("", "derivative", "Enter derivative mode")
